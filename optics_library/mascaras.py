...
def funcion_sinc(tamaño_disco, xx,yy, centro = [0,0]):
    transmitancia = np.sin(np.pi * (xx-centro[0])/tamaño_disco)/(np.pi*(xx-centro[0])/tamaño_disco) * np.sin(np.pi * (yy-centro[1])/tamaño_disco)/(np.pi*(yy-centro[1])/tamaño_disco)
    return transmitancia
...
def rejilla_difraccion(espejos, xx, yy, angulo = 90 ,opacidad_clara=1, opacidad_oscura=0):
    ''' 
    ...
    if angulo == 90:  #Franjas verticales porque la tangente no está definida en 90 grados
        mascara = np.where((xx // (espejos * 5.4E-6)) % 2 == 0, opacidad_clara, opacidad_oscura)
    else:
        pendiente = np.tan(angulo * np.pi/180)           #Convertir ángulo a pendiente
        rejilla = (yy - pendiente * xx) // (espejos * 5.4E-6)     
